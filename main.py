...
import telepot
bot=telepot.Bot('7016928382:AAH0oTn4yUhYNbluYiowThD56gg4xaNJzyk')
rec=1      
class IdData:
...
def main(args):
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            save_dir = "matched_frames"  # Directory to save matched frames
            ...
            out_video = None  # Initialize VideoWriter object
                ...
                if len(face_patches) > 0:
                    ...
                    for bb, landmark, matching_id, dist in zip(
                        ...
                        if matching_id is None:
                            ...
                            bot.sendMessage('7205411951', str('THE PERSON FOUND AT https://maps.app.goo.gl/g1L25o3xPBGht1iDA is {}, {}'.format(matching_id,aa)))
                            frame_filename = os.path.join(save_dir, f"frame_{frame_count}.jpg")
                            cv2.imwrite(frame_filename, frame)
                            print(f"Saved frame {frame_count} with matching ID: {matching_id}")
                            frame_count += 1

                            # If VideoWriter object is not initialized, initialize it
                            if out_video is None:
                                current_datetime = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                                video_filename = f"matched_video_{current_datetime}_{matching_id}.avi"
                                output_video_path = os.path.join(save_dir, video_filename)
